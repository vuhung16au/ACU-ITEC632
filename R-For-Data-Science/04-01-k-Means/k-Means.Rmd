---
title: "K-Means Clustering - Ames Housing Dataset"
...
```{r cluster-characteristics-plot}
# Create cluster characteristics plots
par(mfrow = c(2, 3), mar = c(4, 4, 3, 2))
...
This analysis demonstrates the use of k-means clustering for feature engineering in the context of the Ames Housing dataset. The key findings include:

1. **Feature Selection**: Five key housing features were selected for clustering based on their relevance to house characteristics.

2. **Optimal Clustering**: The analysis used k=10 clusters as specified in the reference notebook, with methods to determine optimal k provided.

3. **Feature Engineering**: Two types of new features were created:
   - **Cluster Labels**: Categorical features indicating which cluster each house belongs to
   - **Cluster-Distance Features**: Continuous features representing distances to each cluster center

4. **Model Impact**: The analysis shows how these engineered features can improve model performance, with both cluster labels and distance features providing additional predictive power.

5. **Visualization**: Comprehensive visualizations help understand cluster characteristics and the distribution of houses across different clusters.

The k-means clustering approach provides a powerful method for creating new features that capture complex patterns in the data that might not be apparent from individual features alone.
